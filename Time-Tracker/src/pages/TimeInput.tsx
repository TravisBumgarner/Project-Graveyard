...
const TimeInput = () => {
    const { dispatch, state: { currentlyTracked } } = useContext(context)
    ...
    const startTimer = useCallback(async () => {
        ...
        if (newProjectTitle.length > 0) {
            newProjectId = (await addProject()).id
        }

        dispatch({ type: 'UPDATE_CURRENTLY_TRACKED', payload: { startTime: moment().toISOString(), projectId: newProjectId ?? selectedProjectId } })
    }, [selectedProjectId, dispatch, newProjectTitle, addProject])
    ...
    const triggerEditModal = useCallback((projectEntryId: string) => {
        dispatch({
            type: 'SET_ACTIVE_MODAL',
            payload: {
                id: ModalID.EDIT_ENTRY_MODAL,
                projectEntryId
            }
        })
    }, [dispatch])
    ...
    const cancelTimer = useCallback(() => {
        ...
        setPercentFocus(100)
        setDetails('')
        dispatch({ type: 'UPDATE_CURRENTLY_TRACKED', payload: null })
    }, [dispatch])
    ...
    const endTimer = useCallback(async () => {
        ...
        const newEntry: TProjectEntry = {
            start: formatDateKeyLookup(moment(currentlyTracked.startTime)),
            end: formatDateKeyLookup(moment()),
            id: uuidv4(),
            projectId: currentlyTracked.projectId,
            details
        }
        ...
        setDetails('')
        dispatch({ type: 'UPDATE_CURRENTLY_TRACKED', payload: null })
    }, [details, currentlyTracked, dispatch])
                                ...
                                {projectEntries?.map((entry) => {
                                    ...
                                    return (
                                        < TableRow key={entry.id} >
                                            ...
                                            <TableCell>
                                                <IconButton css={{ padding: 0 }} aria-label="edit" onClick={() => { triggerEditModal(entry.id) }}>
